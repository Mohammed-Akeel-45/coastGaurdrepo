...
const TriageView = () => {
  const { reports, verifyReport, debunkReport, demoMode } = useApp();
  ...
  const handleDebunk = async (reportId) => {
    setIsProcessing(true);
    try {
      const result = await debunkReport(reportId);
      if (result.success) {
              ...
              {unverifiedReports.length === 0 ? (
                ...
                unverifiedReports.map((report) => (
                  <div
                    ...
                    className={`bg-gray-800 rounded-lg p-4 border cursor-pointer transition hover:shadow-lg ${
          ...
          <div className="lg:sticky lg:top-6 lg:self-start">
            {selectedReport ? (
              <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-xl">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-sm text-gray-400 font-medium">Hazard Type</label>
                    <p className="text-white font-semibold text-lg">{selectedReport.type_name}</p>
                  </div>

                  <div>
                    <label className="text-sm text-gray-400 font-medium">Reported Time</label>
                    <p className="text-white">
                      {new Date(selectedReport.report_time).toLocaleString('en-IN', {
                        dateStyle: 'full',
                        timeStyle: 'short',
                      })}
                    </p>
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="flex-1 bg-gray-700 rounded-full h-2">
                          <div
                            ...
                            style={{ width: `${selectedReport.relevance_score}%` }}
                          ></div>
                        </div>
